...
response_time_T_mod=cbind(EQGEFRAME[EQGEFRAME$scramb==1 & !is.na(EQGEFRAME$Prop),],predict(model_6))
...
createLOOstruct=function(frame){
  ...
  for (i in 1:length(psvec)){
    assign(envsin[i],frame[frame$ps!=psvec[i] & !is.na(frame$Prop),], envir = x)
    assign(envsout[i],frame[frame$ps==psvec[i],], envir = x)
  ...
  #  assign(fitEQ[i],lmer(Prop ~ AOI*EQ*(ot1+ot2)+(1 + ot1 + ot2 |ps),data = get(envsin[i],envir=x), REML = FALSE), envir = x)
   # assign(fitEQGE[i],lmer(Prop ~ AOI*EQ*GE*(ot1+ot2)+(1 + ot1 + ot2 |ps),data = get(envsin[i],envir=x), REML = FALSE), envir = x)
    assign(predfit1[i],qplot(predict(get(fit1[i],envir = x),get(envsout[i],envir=x),allow.new.levels = TRUE),get(envsout[i]$Prop,envir=x)), envir = x)
    print(i)
  }
  return(x)
}
